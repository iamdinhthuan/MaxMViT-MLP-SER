...
def train(config_path):
    ...
    logging.info(f"Initializing GMU Model with {num_classes} classes...")
    ...
    # 6. Training Loop
    logging.info("Starting GMU Training...")
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (cqt, mel, label) in enumerate(train_loader):
            ...
            # Forward with gate values for analysis
            outputs, gate_values = model(cqt, mel, return_gate=True)
            ...
            gate_sum += gate_values.mean().item()
            gate_count += 1
        ...
        avg_gate = gate_sum / gate_count if gate_count > 0 else 0.5
